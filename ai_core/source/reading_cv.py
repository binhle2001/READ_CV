...
client = OpenAI(api_key=get_env_var("openai", "API_KEY"))
app = FastAPI()
...
def extract_candidate_info(contents):
    ...
    SYSTEM_CONTENT = f"""Tokyo Tech Lab Bot is a bot designed to extract information from the content of PDF files. Tokyo Tech Lab Bot is capable of:
        - Please rewrite this with correct spelling. For example: "Đ ạ i h ọ c Qu ố c Gia Hà N ộ i" is changed to "Đại học Quốc Gia Hà Nội".
        - Extracting information such as name, position applied for, email, work experience/projects undertaken, education, skills.
        - Providing responses in the form of a dictionary as follows:
        {info_format}
        - Do not use external knowledge beyond the information provided in the PDF files. If information is not provided, write "null". If a field has no information, return the entire field as null."""
    messages = [
            {"role": "system", "content": SYSTEM_CONTENT},
            {"role": "user", "content": "The PDF file content is "+ pdf_info.text},
        ]
